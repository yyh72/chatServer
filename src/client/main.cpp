            ...
            else
            {
                ...
                else
                {
                    ...
                    else // 登录成功
                    {
                        ...
                        // 记录当前用户的好友列表信息
                        if (responsejs.contains("friends")) // 判断有没有friends
                        {
                            // 初始化
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        int msgtyoe = js["msgid"].get<int>();
        if (ONE_CHAT_MSG == msgtyoe)
        {
            cout << js["time"].get<string>() << " [" << js["id"] << "]" << js["name"].get<string>() << " said: " << js["msg"].get<string>() << endl;
            continue;
        }

        if (GROUP_CHAT_MSG == msgtyoe)
        {
            cout << "群消息[" << js["groupid"] << "]:" << js["time"].get<string>() << " [" << js["id"] << "]" << js["name"].get<string>() << " said: " << js["msg"].get<string>() << endl;
            continue;
        }
...
unordered_map<string, string> commandMap =
    {
        ...
        {"groupchat", "群聊,格式:groupchat:grouid:message"},
